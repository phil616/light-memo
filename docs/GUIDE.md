...
点击备忘录页面顶部的“同步”可立即刷新当前搜索、标签和分页下的备忘录及标签列表；失败后可再次点击重试。同步不会提交正在编辑的内容。

编辑器的“清空全部正文”仅清空正文，保留标题与标签，点击“保存”后才更新服务器。

新建和编辑时，未保存内容会自动写入当前浏览器的 localStorage，按后端地址及备忘录分别保存。关闭编辑器、刷新页面、退出登录或授权失效后，重新打开同一备忘录的编辑器（新建草稿则点击“新建”）即可恢复。保存成功后清除对应草稿，保存失败则继续保留。草稿不参与任何网络请求或同步；只有主动点击“保存”才提交编辑器内容。不同设备或浏览器无法共享草稿，清理站点数据会删除草稿。本地存储不可用或空间不足时，编辑器会显示保存失败提示。
